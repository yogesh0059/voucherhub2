...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (u.role === "admin") {
      ...
      axios.get(`http://localhost:5000/api/coupons?addedBy=${u.email}`)
        .then(res => setYourCoupons(res.data));
    } else {
      // User side: Visited & Purchased
      axios.get(`http://localhost:5000/api/user-visited?user=${u.email}`)
        .then(res => setVisitedCoupons(res.data));
      axios.get(`http://localhost:5000/api/user-purchased?user=${u.email}`)
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`http://localhost:5000/api/coupons/${id}`)
      .then(() => setYourCoupons(yourCoupons.filter(c => c._id !== id)));
  };
  const handleEdit = (id) => {
    // Yahan edit modal ya page open karao (yahan sirf alert demo)
          ...
          <div style={{display:"flex", flexWrap:"wrap", gap:"24px"}}>
            {yourCoupons.map(c => (
              <div key={c._id} style={{boxShadow:'0 1.5px 12px #ddd',borderRadius:12,padding:12,width:236,position:'relative'}}>
                <img src={`http://localhost:5000${c.thumbnail || c.images[0]}`} style={{width:'100%',height:90,objectFit:'cover',borderRadius:8}} alt={c.title}/>
                <h4>{c.title}</h4>
